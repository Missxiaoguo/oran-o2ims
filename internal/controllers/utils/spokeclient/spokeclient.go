...
func EnsureSpokeClient(
	...
	entry, cached := spokeClients[clusterName]
	...
	if err := hubClient.Get(ctx, types.NamespacedName{
		...
		if errors.IsNotFound(err) {
			return nil, false, ctlrutils.NewInputError(
	...
	spokeClient, err := newSpokeClientFunc(apiServerURL, string(tokenSecret.Data["token"]), tokenSecret.Data["ca.crt"], spokeScheme)
	if err != nil {
		return nil, false, err
	}
...
func refreshCachedSpokeClient(
	...
	if err != nil {
		if errors.IsNotFound(err) {
			spokeClientsMu.Lock()
			delete(spokeClients, clusterName)
			spokeClientsMu.Unlock()
	...
	spokeClient, err := newSpokeClientFunc(
		apiServerURL, string(tokenSecret.Data["token"]), tokenSecret.Data["ca.crt"], spokeScheme)
	if err != nil {
...
func CleanupSpokeAccess(
	...
	if err := hubClient.Get(ctx, types.NamespacedName{
		...
		if err := hubClient.Delete(ctx, msa); client.IgnoreNotFound(err) != nil {
	...
	if err := hubClient.Get(ctx, types.NamespacedName{
		...
		if err := hubClient.Delete(ctx, mw); client.IgnoreNotFound(err) != nil {
