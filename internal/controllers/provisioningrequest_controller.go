...
func (t *provisioningRequestReconcilerTask) run(ctx context.Context) (ctrl.Result, error) {
	...
	if utils.IsClusterZtpDone(t.object) {
		...
		if shouldUpgrade ||
			...
			upgradeCtrlResult, err := t.handleUpgrade(ctx, renderedClusterInstance.GetName())
			if err != nil {
				return requeueWithError(err)
			}
			if !upgradeCtrlResult.IsZero() {
				// Upgrade process is still in progress, requeue
				return upgradeCtrlResult, nil
			}
		}
	}
