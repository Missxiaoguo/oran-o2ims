...
func (t *provisioningRequestReconcilerTask) handleUpgrade(ctx context.Context, clusterName string) (ctrl.Result, bool, error) {
	...
	if err != nil && errors.IsNotFound(err) {
		...
		if err := utils.UpdateK8sCRStatus(ctx, t.client, t.object); err != nil {
			return nextReconcile, proceed, fmt.Errorf("failed to update ClusterRequest CR status: %w", err)
		}

	} else if err != nil {
		return nextReconcile, proceed, fmt.Errorf("error getting IBGU: %w", err)
	}
	...
	if isIBGUProgressing(ibgu) {
		...
		if clusterTemplate.Spec.Release == ibgu.Spec.IBUSpec.SeedImageRef.Version {
			...
			// TODO: move the release validation to the ClusterTemplate and remove this.
